...
当前验证问题与搜索问题有重叠，在完成独立数据切分前只称“正向复现验证”，不称“独立留出验证”。
...
- 正向验证尚未严格独立，需要新增 search/validation 数据切分。

### 后续动作

- 拆分搜索与最终验证问题集。
- 在代码和报告 schema 中逐步把历史字段 `lift` 迁移为 `reference_separation`。
- 修复原始 CLI 的 LOW/inconclusive 冲突。
- 以 Qwen2.5-0.5B 为第一个跨架构验证对象。
