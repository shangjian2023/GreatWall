...
6. 完成冻结后盲测，再增加其他条件类型或 Pythia-70M；旧隐式 benchmark
   与平台代码只作回归，不再优先扩展。
7. OPT-125M 组员配对已暴露 Top-4 原始排名漏掉高支持候选族的问题。取得该配对的
   backdoor/clean Adapter 后，先冻结只依赖候选族结构的代表选择规则，再重跑 probe；不得
   用训练目标事后指定 rank 5，也不得把 GPT-2 阈值迁移结果包装成 OPT-125M 校准。
