...
1. 保留 ADR-0031 的纯论文判据失败结论：不得把 10,000 条固定步排序改善描述为
   ...
   良性序列生成、软提示初始化与检查步位置。
2. 保持 `gpt2-family-support-dev-v2` 冻结：5 clean 最大族支持为 4、2、3、2、4，2 个
   `register_condition` 后门为 7、8；同候选双条件开发结果为 TP=2、TN=5、FP=0、FN=0。
3. 预留至少 1 个新的 clean 和 1 个新的后门种子作为冻结参数后的盲测，不得用于继续选阈值。
4. 盲测运行同时记录最大优化对数似然差、新输入对数似然差和软触发完整前缀
   复现率；这些字段在阈值预注册前只作辅助观察，不得加入现役结论。
